...
Dalla caccia sistematica dell'8 agosto erano rimaste **30 segnalazioni non verificate** (gli agenti hanno esaurito il tempo). Erano solo un numero: adesso stanno in **[`CACCIA-IMPOSTORE.md`](CACCIA-IMPOSTORE.md)**, ricontrollate una per una contro il codice. Trenta segnalazioni sono diciassette difetti distinti — agenti diversi hanno trovato più volte la stessa cosa — e ne **restano aperti nove**: il colpo di coda che dice «Ti hanno beccato» a tutti, la partita in quattro con due impostori che nasce già vinta, il tasto «Annulla» in mano a chi non gioca, la nota d'apertura che promette un numero che poi si vota, la parola che non si può rivedere durante il voto, i voti orfani di ogni giro, `schema.sql` che rilanciato resuscita le funzioni vecchie, `paga()` senza guardia, e la RPC lenta che riscrive sopra il realtime.

Più alcuni "raro" confermati:
...
- ~~Con due impostori beccati insieme, entrambi possono scrivere la parola ma vale solo la prima~~ — **fatto, ma la decisione è diversa da quella che era scritta qui.** Non si decide *chi* tenta: tentano tutti e due e vale la prima parola che parte, perché sono una squadra seduta allo stesso tavolo e togliere la scelta a due che possono parlarsi è peggio del problema. Quello che era rotto era il **silenzio**: `tenta_colpo`, se il tentativo c'è già, restituisce la riga senza sollevare niente, quindi chi arrivava secondo si ritrovava il finale calcolato sulla parola di un altro e nessuno gli diceva perché. Ora la casella lo dice **prima** (*"Vi hanno beccati in due"*) e chi arriva secondo lo legge **dopo**, in un avviso che non se ne va da solo.
