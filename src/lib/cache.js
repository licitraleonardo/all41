...
export function conCache(chiave, lettura) {
  return async (...argomenti) => {
    ...
    if (!k) return lettura(...argomenti)

    try {
      const dati = await lettura(...argomenti)
      inCache(k, dati)
      return dati
    } catch (e) {
      if (!sembraRete(e, navigator.onLine !== false)) throw e

      const copia = daCache(k)
      if (!copia) throw e

      console.info(`[all41] senza rete: "${k}" dalla copia del ${copia.quando}`)
      return copia.dati
    }
  }
}
